...
export const createBookingAuthSchema = z.object({
  ...
  date: z.string().transform((str) => {
    ...
    if (str.match(/^\d{4}-\d{2}-\d{2}$/)) {
      // Formato de data simples - criar data no fuso horário do Brasil (UTC-3)
      const [year, month, day] = str.split('-').map(Number)
      // Criar data no horário do Brasil (UTC-3)
...
export type CreateBookingAuthDto = z.infer<typeof createBookingAuthSchema>

export class CreateBookingAuthDtoSwagger {
  @ApiProperty({
